        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('account.update', $account) }}" x-data="{ role: '{{ old('role', $account->role) }}' }">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-6">
                            ...
                            <select name="role" id="role" x-model="role"
                                ...
                                @foreach($availableRoles as $roleOption)
                                    <option value="{{ $roleOption }}" {{ old('role', $account->role) == $roleOption ? 'selected' : '' }}>
                            ...
                            <select name="child_username" id="child_username" 
                                ...
                                @foreach($availableChildren as $child)
                                    <option value="{{ $child->username }}" 
                                        {{ old('child_username', $account->children->first()?->username) == $child->username ? 'selected' : '' }}>
